...
st.set_page_config(
    page_title=f"{APP_NAME}", page_icon="🩺", layout="wide"
)
...
def render_chat_bubble(role, content):
    st.markdown(
        f"""
        <div class='hia-chat-row {role}'>
            <div class='hia-bubble {role}'>{content}</div>
        </div>
        """,
        unsafe_allow_html=True,
    )
...
def main():
    ...
    # Show header with app branding and user greeting
    show_header()
    ...
    if st.session_state.get("current_session"):
        st.markdown(
            f"<span class='hia-badge'>📊 {st.session_state.current_session['title']}</span>",
            unsafe_allow_html=True,
        )
        st.write("")
